     ...
     return () => {
      ...
      if(props){
        ...
        <div id={"popup1"+props.id} class="overlay">
                <div class="popup" >
                        <h2>Comment</h2>
                        <a class="close" onClick={()=>dispatch({type:bogusTriggerAction,payload:null})
} href="##">&times;</a>
      ...
      return(<>
        ...
        <div id="popup1" class="overlay">
                <div class="popup">
                        <h2>Here i am</h2>
                        <a class="close" href="##" onClick={()=> document.body.style.color = 'green'
...
//     const [commentData, setCommentData] = useState({post_id:props.post_id, author_id:props.author_id})
  const [commentData, setCommentData] = useState({})
    ...
    const {state: {postComment}} = useContext(store);
    const {state:{bogus}} = useContext(store)
let pcont = ""
...
try {
    ...
    if(bb){
            if(bogus.type==="comment-reply"){
                const rdata  = {to_id:"null", comment_id:bogus.comment_id, comment:commentData.comment, author_id:props.author_id, type:bogus.type}
               ...
               if(replyResult){
                 ...
                 let container = document.querySelector("#appendrep"+replyResult.data.postcomment)
                 container.style.display="flex"
                //  dispatch({type:newCommentAction,payload:"heya"});
                 let comment = document.querySelector("#appendrep_comment"+replyResult.data.postcomment)
                 let username = document.querySelector("#appendrep_username"+replyResult.data.postcomment)
                 let img = document.querySelector("#appendrep_img"+replyResult.data.postcomment)
                ...
                const rrdata  = {reply_id:bogus.reply_id, to_id:bogus.reply_id, comment:commentData.comment, author_id:props.author_id, parent_id:bogus.parent_id, type:bogus.type}
               ...
               if(replyResult){
                 ...
                 let container = document.querySelector("#appendreprep"+replyResult.data.postcomment)
                 container.style.display="flex"
                //  dispatch({type:newCommentAction,payload:"heya"});
                 let comment = document.querySelector("#appendreprep_comment"+replyResult.data.postcomment)
    ...
    setCommentData({...commentData, post_id:postComment, author_id:props.author_id})
    console.log("comment data::::", commentData);
//     console.log("postcomment:::", postComment)
    let data={...commentData,post_id:postComment, author_id:props.author_id }
...
const LoadComment = (props) =>{
        ...
        const {
            state: {postComment}, dispatch, } = useContext(store);
            const { state: {commentTrigger} } = useContext(store);
            const { state: {newComment} } = useContext(store);
        const { state: {bogus} } = useContext(store);
        ...
        return(
                ...
                {render? (<>
                        <div  class="contenta"  >
                           ...
                           <div className="comment" id={"appendcom"+props.post_id} style={{display:"none"}}>
                <div className="comment-avatar">
                <img src="" alt="" id={"appendcom_img"+props.post_id}></img>
                </div>
                <div className="comment-user-data">
                <div className="username" id={"appendcom_username"+props.post_id}>

                </div>

                <div className="comment-text">
                 <span className="comment-span" id={"appendcom_comment"+props.post_id}></span>
                ...
                {commentList.map((item, key)=>{
                   return  <CommentCard post_id={props.post_id} key={key} data={item}/>
                })}
                </div>
                </>):(<>
                         <div class="contenta" id={"append"+props.post_id}><CommentCard/></div>
 ...
 const CommentCard = (props) =>{
        ...
        const { state: {postComment}, dispatch} = useContext(store);
        const { state: {newComment} } = useContext(store);
        const { state: {newReply} } = useContext(store);
        ...
        const getReply =  async(e,comment_id) =>{
         ...
         if(props.data){
            ...
            return(
                ...
                <div className="comment-text">
                 <span className="comment-span" id={props.data.id} >{comment}</span>
                  <span className="replytag" onClick={(e)=>SendReply(props.data.id, props.data.author.username)}>reply</span>


                  <div className="comment" style={{display:"none"}} id={"appendrep"+props.data.id}> 
                       <div className="comment-avatar">
                       <img src="" alt="" id={"appendrep_img"+props.data.id}></img>
                       </div>
                 <div className="comment-user-data">
                  <div className="username" style={{fontSize:"0.8rem"}} id={"appendrep_username"+props.data.id}>
                   </div>
                        <div className="comment-text">
                        <span className="comment-span" id={"appendrep_comment"+props.data.id}></span>
                        <span className="replytag"></span>
                        </div>
                </div></div>
                ...
                {replyData && replyData.map((item, key)=>{
                    ...
                    <div id={"appendreprep"+props.data.id} style={{display:"none", paddingLeft:"5px"}} >
                <div className="comment-avatar" >
                <img src="" alt="" id={"appendreprep_img"+props.data.id}></img>
                </div> &nbsp; 
<p>
        <span className="reply-username" id={"appendreprep_username"+props.data.id}>a</span> 
<span className="repto"> &nbsp; replied</span>
<span className="reply-username" id={"appendreprep_to_username"+props.data.id}>b</span>
<span  className="reply-comment" id={"appendreprep_comment"+props.data.id}>heya</span>

</p>  </div>   
                {replyAvailable && <p className="comment-rtext" onClick={(e)=> getReply(e,props.data.id)}> {view} ({replyLength && replyLength})</p>}          
                 </div>
                </div>
        </div>
            );}else{
...
const Reply =(props)=>{
        ...
        const SendReplyTo =  async(reply_id, who, parent_id) =>{
                ...
                dispatch({type:bogusTriggerAction,payload:{placeholder:placeholderText,reply_id:reply_id, postComment:postComment, type:"reply-reply", parent_id:parent_id}});
                 ...
                 <div className="comment-user-data">
                  <div className="username" style={{fontSize:"0.8rem"}}>
                     { reply.author.username} 
                        ...
                        <div className="comment-text">
                        <span className="comment-span" id={props.data.id} >{reply.comment}</span>
                        <span className="replytag" onClick={(e)=>SendReplyTo(reply.id, reply.author.username, reply.postcomment)}>reply</span>
                        </div>
