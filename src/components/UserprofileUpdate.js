...
import {userDetailAction, bogusTriggerAction} from "../stateManagement/actions";
...
const UserProfileUpdate =(props)=>{
  ...
  const propicSubmit = async (e) => {
    ...
    const method1 = "post"
    const result = await axiosHandler({method:method1, url:PROFILE_PIC_URL,token, data}).catch(
      e => console.log(e)
    )
    setUploading(false)
    if(result){
