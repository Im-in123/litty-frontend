...
const ChatPage = (props) => {
  ...
  const getOtherProfile = async (props) => {
    ...
    if (gp) {
      ...
      dispatch({
        ...
        payload: {
          username: gg.user.username,
          img: BASE_URL1 + gg.user.user_picture,
          timeout: gg.user.id,
        },
      });
  ...
  const getChat = (username, img, timeout) => {
    ...
    dispatch({
      type: activeChatUserAction,
      payload: { username: username, img: img, timeout: timeout },
    });
  };
    ...
    if (item.other) {
      ...
      return (
        <>
          <div
            className={
              item.other.username === activeChatUser.username
            ...
            onClick={() =>
              getChat(
                item.other.username,
                LOCAL_CHECK
                  ? item.other.user_picture
                  : item.other.user_picture_url,
                item.other.id
              )
            }
          >
            ...
            <div class="timeago">{/* <p>20 mins ago</p> */}</div>
          </div>
        </>
      );
    }
        ...
        <>
          <div class="msg">
            <div
              key={item.id}
              className={
                item.sender === userDetail.user.username
                  ? "user friend t"
                  : "user me t"
              }
            >
  ...
  if (loading) {
    return <div class="content">loading...</div>;
  }
  return (
    <div class="content">
      <aside>
      ...
      <div class="midcont">
        <div class="head">
          <div class="avatarbox">
            <div class="avatar_overlay">
              <img src={activeChatUser.img} />
            </div>
          </div>
          <h4>{activeChatUser.username}</h4>
          <div class="configschat">
